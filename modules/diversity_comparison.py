...
if (not(us is None)):
    ...
    listofuserid = []
    #res = open("RISULTATI_COMPARISON_INTERSECTIONRANK.txt","w+")
    liste = open("listeacquisti.txt","w+")
    #print("The number of recommendations with intersection over 80% are: ", len(SetIdOverThres))
    ...
    for value in uniqueValue:
    	...
    	for index in range(len(listofper)):
    		if(value == listofper[index]):
    			...
    			user_id = data.loc[data["user_id"] == index, "UserId"].iloc[0]
    			listofuserid.append(user_id)
    			item_purchased = data.loc[data["user_id"] == index, "ItemId"]
    			item_purchased = list(item_purchased)
    	#print("Percentage ", value ," reapet N. ", SPerCount, " Times( ", SPerCount * 100 / len(listofper),"% compare with the total length )")
